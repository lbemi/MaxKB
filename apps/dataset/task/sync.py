...
@celery_app.task(
    base=QueueOnce, once={"keys": ["dataset_id"]}, name="celery:sync_web_dataset"
)
def sync_web_dataset(dataset_id: str, url: str, selector: str):
    try:
        ...
        ForkManage(url, selector.split(" ") if selector is not None else []).fork(
            2, set(), get_save_handler(dataset_id, selector)
        )
        max_kb.info(f"结束--->结束同步web知识库:{dataset_id}")
    except Exception as e:
        max_kb_error.error(
            f"同步web知识库:{dataset_id}出现错误{str(e)}{traceback.format_exc()}"
        )
