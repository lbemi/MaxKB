...
from common.response.result import (
...
class Dataset(APIView):
    ...
    class SyncWeb(APIView):
        ...
        @has_permissions(
            ...
            PermissionConstants.DATASET_EDIT,
            compare=CompareConstants.AND,
        )
        def put(self, request: Request, dataset_id: str):
            return result.success(
                DataSetSerializers.SyncWeb(
                    data={
    ...
    class CreateQADataset(APIView):
        ...
        @has_permissions(
            PermissionConstants.DATASET_CREATE, compare=CompareConstants.AND
        )
        def post(self, request: Request):
            return result.success(
                DataSetSerializers.Create(data={"user_id": request.user.id}).save_qa(
                    {
    ...
    class CreateWebDataset(APIView):
        ...
        def post(self, request: Request):
            return result.success(
                DataSetSerializers.Create(data={"user_id": request.user.id}).save_web(
                    request.data
                )
            )
    ...
    class Application(APIView):
        ...
        def get(self, request: Request, dataset_id: str):
            return result.success(
                DataSetSerializers.Operate(
                    data={"id": dataset_id, "user_id": str(request.user.id)}
                ).list_application()
            )
    ...
    def get(self, request: Request):
        d = DataSetSerializers.Query(
            data={**request.query_params, "user_id": str(request.user.id)}
        )
        d.is_valid()
    ...
    def post(self, request: Request):
        return result.success(
            DataSetSerializers.Create(data={"user_id": request.user.id}).save(
                request.data
            )
        )
    ...
    class HitTest(APIView):
        ...
        @action(methods="GET", detail=False)
        @swagger_auto_schema(
            operation_summary="命中测试列表",
            operation_id="命中测试列表",
            manual_parameters=CommonApi.HitTestApi.get_request_params_api(),
            responses=result.get_api_array_response(
                CommonApi.HitTestApi.get_response_body_api()
            ),
            tags=["知识库"],
        )
        @has_permissions(
    ...
    class Embedding(APIView):
        ...
        def put(self, request: Request, dataset_id: str):
            return result.success(
                DataSetSerializers.Operate(
                    data={"id": dataset_id, "user_id": request.user.id}
                ).re_embedding()
            )
    ...
    class Export(APIView):
        ...
        def get(self, request: Request, dataset_id: str):
            return DataSetSerializers.Operate(
                data={"id": dataset_id, "user_id": request.user.id}
            ).export_excel()
    ...
    class Operate(APIView):
        ...
        @action(methods="GET", detail=False)
        @swagger_auto_schema(
            operation_summary="查询知识库详情根据知识库id",
            operation_id="查询知识库详情根据知识库id",
            manual_parameters=DataSetSerializers.Operate.get_request_params_api(),
            responses=get_api_response(
                DataSetSerializers.Operate.get_response_body_api()
            ),
            tags=["知识库"],
        )
        @has_permissions(
        ...
        def get(self, request: Request, dataset_id: str):
            return result.success(
                DataSetSerializers.Operate(
                    data={"id": dataset_id, "user_id": request.user.id}
                ).one(user_id=request.user.id)
            )

        @action(methods="PUT", detail=False)
        @swagger_auto_schema(
            operation_summary="修改知识库信息",
            operation_id="修改知识库信息",
            manual_parameters=DataSetSerializers.Operate.get_request_params_api(),
        ...
        def put(self, request: Request, dataset_id: str):
            return result.success(
                DataSetSerializers.Operate(
                    data={"id": dataset_id, "user_id": request.user.id}
                ).edit(request.data, user_id=request.user.id)
            )
    ...
    class Page(APIView):
        ...
        @swagger_auto_schema(
            ...
            responses=get_page_api_response(
                DataSetSerializers.Query.get_response_body_api()
            ),
            tags=["知识库"],
        )
