...
class WebDocument(APIView):
    ...
    @has_permissions(
        lambda r, k: Permission(
            group=Group.DATASET, operate=Operate.MANAGE, dynamic_tag=k.get("dataset_id")
        )
    )
    def post(self, request: Request, dataset_id: str):
        return result.success(
            DocumentSerializers.Create(data={"dataset_id": dataset_id}).save_web(
...
class Document(APIView):
    ...
    @action(methods=["GET"], detail=False)
    @swagger_auto_schema(
        operation_summary="文档列表",
        operation_id="文档列表",
        manual_parameters=DocumentSerializers.Query.get_request_params_api(),
        responses=result.get_api_response(
            DocumentSerializers.Query.get_response_body_api()
        ),
        tags=["知识库/文档"],
    )
    @has_permissions(
        lambda r, k: Permission(
            group=Group.DATASET, operate=Operate.USE, dynamic_tag=k.get("dataset_id")
        )
    )
    ...
    class BatchEditHitHandling(APIView):
        ...
        @action(methods=["POST"], detail=False)
        @swagger_auto_schema(
            operation_summary="批量修改文档命中处理方式",
            operation_id="批量修改文档命中处理方式",
            request_body=DocumentApi.BatchEditHitHandlingApi.get_request_body_api(),
            manual_parameters=DocumentSerializers.Create.get_request_params_api(),
    ...
    class Batch(APIView):
        ...
        @action(methods=["POST"], detail=False)
        @swagger_auto_schema(
            operation_summary="批量创建文档",
            operation_id="批量创建文档",
            request_body=DocumentSerializers.Batch.get_request_body_api(),
            manual_parameters=DocumentSerializers.Create.get_request_params_api(),
        ...
        def post(self, request: Request, dataset_id: str):
            return result.success(
                DocumentSerializers.Batch(data={"dataset_id": dataset_id}).batch_save(
                    request.data
                )
            )
        ...
        def put(self, request: Request, dataset_id: str):
            return result.success(
                DocumentSerializers.Batch(data={"dataset_id": dataset_id}).batch_sync(
                    request.data
                )
            )
        ...
        def delete(self, request: Request, dataset_id: str):
            return result.success(
                DocumentSerializers.Batch(data={"dataset_id": dataset_id}).batch_delete(
                    request.data
                )
            )
    ...
    class Split(APIView):
        ...
        def post(self, request: Request):
            ...
            ds = DocumentSerializers.Split(data=split_data)
