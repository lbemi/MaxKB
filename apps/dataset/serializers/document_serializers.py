...
from embedding.task.embedding import (
...
class DocumentEditInstanceSerializer(ApiMixin, serializers.Serializer):
    ...
    directly_return_similarity = serializers.FloatField(
        ...
        max_value=2,
        min_value=0,
        error_messages=ErrMessage.float("直接返回分数"),
    )

    is_active = serializers.BooleanField(
        required=False, error_messages=ErrMessage.boolean("文档是否可用")
    )
...
class DocumentWebInstanceSerializer(ApiMixin, serializers.Serializer):
    source_url_list = serializers.ListField(
        ...
        child=serializers.CharField(
            required=True, error_messages=ErrMessage.char("文档地址")
        ),
        error_messages=ErrMessage.char("文档地址列表"),
    )
...
class DocumentInstanceSerializer(ApiMixin, serializers.Serializer):
    ...
    def get_request_body_api():
        return openapi.Schema(
            ...
            properties={
                "name": openapi.Schema(
                    type=openapi.TYPE_STRING, title="文档名称", description="文档名称"
                ),
...
class DocumentSerializers(ApiMixin, serializers.Serializer):
    class Export(ApiMixin, serializers.Serializer):
        ...
        def export(self, with_valid=True):
            ...
            if self.data.get("type") == "csv":
                file = open(
                    os.path.join(
                        PROJECT_DIR, "apps", "dataset", "template", "csv_template.csv"
                    ),
                    "rb",
                )
                content = file.read()
    ...
    class Migrate(ApiMixin, serializers.Serializer):
        ...
        target_dataset_id = serializers.UUIDField(
            required=True, error_messages=ErrMessage.char("目标知识库id")
        )
        document_id_list = serializers.ListField(
            required=True,
            error_messages=ErrMessage.char("文档列表"),
        ...
        def migrate(self, with_valid=True):
            ...
            document_list = QuerySet(Document).filter(
                dataset_id=dataset_id, id__in=document_id_list
            )
            paragraph_list = QuerySet(Paragraph).filter(
                dataset_id=dataset_id, document_id__in=document_id_list
            )
            ...
            target_handle_problem_list = [
                self.get_target_dataset_problem(
                    target_dataset_id,
            ...
            QuerySet(ProblemParagraphMapping).bulk_update(
                problem_paragraph_mapping_list, ["problem_id", "dataset_id"]
            )
            # 修改文档
            if (
                dataset.type == Type.base.value
                and target_dataset.type == Type.web.value
            ):
                ...
                target_dataset.type == Type.base.value
                and dataset.type == Type.web.value
            ):
                document_list.update(
                    dataset_id=target_dataset_id, type=Type.base, meta={}
                )
            else:
            ...
            if model_id:
                ...
                QuerySet(Document).filter(id__in=document_id_list).update(
        ...
        @staticmethod
        def get_target_dataset_problem(
    ...
    class Query(ApiMixin, serializers.Serializer):
        ...
        hit_handling_method = serializers.CharField(
            required=False, error_messages=ErrMessage.char("命中处理方式")
        )
        ...
        def get_query_set(self):
            ...
            if (
                "hit_handling_method" in self.data
                and self.data.get("hit_handling_method") is not None
            ):
                query_set = query_set.filter(
        ...
        def list(self, with_valid=False):
            ...
            return native_search(
                ...
                select_string=get_file_content(
                    os.path.join(
                        PROJECT_DIR, "apps", "dataset", "sql", "list_document.sql"
                    )
                ),
            )
    ...
    class Sync(ApiMixin, serializers.Serializer):
        document_id = serializers.UUIDField(
            required=True, error_messages=ErrMessage.char("文档id")
        )
            ...
            try:
                ...
                if result.status == 200:
                    ...
                    problem_model_list, problem_paragraph_mapping_list = (
                        ProblemParagraphManage(
                            problem_paragraph_object_list, document.dataset_id
    ...
    class Operate(ApiMixin, serializers.Serializer):
        ...
        def refresh(self, with_valid=True):
            ...
            QuerySet(Paragraph).filter(document_id=document_id).update(
                **{"status": Status.queue_up}
            )
            embedding_model_id = get_embedding_model_id_by_dataset_id(
                dataset_id=self.data.get("dataset_id")
            ...
            return openapi.Schema(
                ...
                properties={
                    ...
                    "user_id": openapi.Schema(
                        type=openapi.TYPE_STRING, title="用户id", description="用户id"
                    ),
                    "paragraph_count": openapi.Schema(
                        type=openapi.TYPE_INTEGER,
                        title="文档数量",
                        description="文档数量",
                        default=1,
            ...
            return openapi.Schema(
                ...
                properties={
                    ...
                    "directly_return_similarity": openapi.Schema(
                        type=openapi.TYPE_NUMBER, title="直接返回分数", default=0.9
                    ),
                    "meta": openapi.Schema(
                        type=openapi.TYPE_OBJECT,
                        title="文档元数据",
    ...
    class Create(ApiMixin, serializers.Serializer):
        ...
        def get_paragraph_model(document_model, paragraph_list: List):
            ...
            paragraph_model_dict_list = [
                ...
                ).get_paragraph_problem_model(dataset_id, document_model.id, paragraph)
                for paragraph in paragraph_list
            ]
        ...
        def get_document_paragraph_model(dataset_id, instance: Dict):
            document_model = Document(
                **{
                    ...
                    "meta": (
                        instance.get("meta") if instance.get("meta") is not None else {}
                    ),
                    "type": (
                        instance.get("type")
    ...
    class Split(ApiMixin, serializers.Serializer):
        file = serializers.ListField(
            required=True, error_messages=ErrMessage.list("文件列表")
        )

        limit = serializers.IntegerField(
            required=False, error_messages=ErrMessage.integer("分段长度")
        )

        patterns = serializers.ListField(
            required=False,
            child=serializers.CharField(
                required=True, error_messages=ErrMessage.char("分段标识")
            ),
            error_messages=ErrMessage.uuid("分段标识列表"),
        )

        with_filter = serializers.BooleanField(
            required=False, error_messages=ErrMessage.boolean("自动清洗")
        )
            ...
            if with_valid:
                ...
                if (
                    ...
                    raise AppApiException(
                        500, "命中处理方式必须为directly_return|optimization"
                    )
                self.is_valid(raise_exception=True)
            document_id_list = instance.get("id_list")
            hit_handling_method = instance.get("hit_handling_method")
    ...
    for split_handle in split_handles:
        if split_handle.support(file, get_buffer):
            return split_handle.handle(
                file, pattern_list, with_filter, limit, get_buffer, save_image
            )
    return default_split_handle.handle(
        file, pattern_list, with_filter, limit, get_buffer, save_image
    )
