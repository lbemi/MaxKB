...
from dataset.serializers.problem_serializers import (
...
class ParagraphInstanceSerializer(ApiMixin, serializers.Serializer):
    ...
    content = serializers.CharField(
        required=True,
        error_messages=ErrMessage.char("段落内容"),
        max_length=102400,
        min_length=1,
        allow_null=True,
        allow_blank=True,
    ...
    is_active = serializers.BooleanField(
        required=False, error_messages=ErrMessage.char("段落是否可用")
    )
        ...
        return openapi.Schema(
            ...
            properties={
                ...
                "is_active": openapi.Schema(
                    type=openapi.TYPE_BOOLEAN, title="是否可用", description="是否可用"
                ),
                "problem_list": openapi.Schema(
...
class EditParagraphSerializers(serializers.Serializer):
    title = serializers.CharField(
        required=False,
        max_length=256,
        error_messages=ErrMessage.char("分段标题"),
        allow_null=True,
        allow_blank=True,
    )
    content = serializers.CharField(
        ...
        allow_null=True,
        allow_blank=True,
        error_messages=ErrMessage.char("分段内容"),
    )
    problem_list = ProblemInstanceSerializer(required=False, many=True)
...
class ParagraphSerializers(ApiMixin, serializers.Serializer):
    ...
    content = serializers.CharField(
        required=True, max_length=102400, error_messages=ErrMessage.char("分段内容")
    )
    ...
    class Problem(ApiMixin, serializers.Serializer):
        ...
        def get_request_body_api():
            return openapi.Schema(
                ...
                required=["content"],
                properties={
                    "content": openapi.Schema(type=openapi.TYPE_STRING, title="内容")
                },
            )
        ...
        def get_response_body_api():
            return openapi.Schema(
                ...
                properties={
                    ...
                    "hit_num": openapi.Schema(
                        type=openapi.TYPE_INTEGER,
                        title="命中数量",
                        description="命中数量",
                        default=1,
                    ),
                    "dataset_id": openapi.Schema(
                        type=openapi.TYPE_STRING,
                        title="知识库id",
                        description="知识库id",
                        default="xxx",
    ...
    class Association(ApiMixin, serializers.Serializer):
        ...
        problem_id = serializers.UUIDField(
            required=True, error_messages=ErrMessage.uuid("问题id")
        )
        ...
        def is_valid(self, *, raise_exception=True):
            ...
            if (
                not QuerySet(Paragraph)
                .filter(dataset_id=dataset_id, id=paragraph_id)
                .exists()
            ):
                raise AppApiException(500, "段落不存在")
            if (
                not QuerySet(Problem)
                .filter(dataset_id=dataset_id, id=problem_id)
                .exists()
            ):
    ...
    class Batch(serializers.Serializer):
        ...
        def batch_delete(self, instance: Dict, with_valid=True):
            ...
            QuerySet(ProblemParagraphMapping).filter(
                paragraph_id__in=paragraph_id_list
            ).delete()
    ...
    class Migrate(ApiMixin, serializers.Serializer):
        ...
        target_dataset_id = serializers.UUIDField(
            required=True, error_messages=ErrMessage.uuid("目标知识库id")
        )
        target_document_id = serializers.UUIDField(
            required=True, error_messages=ErrMessage.uuid("目标文档id")
        )
        paragraph_id_list = serializers.ListField(
            required=True,
            error_messages=ErrMessage.char("段落列表"),
        ...
        def is_valid(self, *, raise_exception=False):
            ...
            if (
                ...
                raise AppApiException(
                    5000, f"文档id不存在【{self.data.get('document_id')}】"
                )
            if (
                len(
                    [
                ...
                raise AppApiException(
                    5000, f"目标文档id不存在【{self.data.get('target_document_id')}】"
                )
        ...
        def migrate(self, with_valid=True):
            ...
            paragraph_list = QuerySet(Paragraph).filter(
                dataset_id=dataset_id, document_id=document_id, id__in=paragraph_id_list
            )
            ...
            # 不同数据集迁移
                ...
                paragraph_list.update(
                    dataset_id=target_dataset_id, document_id=target_document_id
                )
                # 修改向量段落信息
                update_embedding_document_id(
    ...
    class Operate(ApiMixin, serializers.Serializer):
        # 段落id
        paragraph_id = serializers.UUIDField(
            required=True, error_messages=ErrMessage.char("段落id")
        )
        # 知识库id
        dataset_id = serializers.UUIDField(
            ...
            if "problem_list" in instance:
                ...
                for update_problem in update_problem_list:
                    if not set([str(row.id) for row in problem_list]).__contains__(
                        ...
                        raise AppApiException(
                            500, update_problem.get("id") + "问题id不存在"
                        )
                # 对比需要删除的问题
                delete_problem_list = (
                    list(
    ...
    class Create(ApiMixin, serializers.Serializer):
        ...
        def save(self, instance: Dict, with_valid=True, with_embedding=True):
            ...
            paragraph_problem_model = self.get_paragraph_problem_model(
                dataset_id, document_id, instance
            )
        ...
        def get_paragraph_problem_model(
            ...
            problem_paragraph_object_list = [
                ProblemParagraphObject(
                    dataset_id, document_id, paragraph.id, problem.get("content")
                )
                for problem in (
    ...
    class Query(ApiMixin, serializers.Serializer):
        ...
        title = serializers.CharField(
            required=False, error_messages=ErrMessage.char("段落标题")
        )
        ...
        def get_response_body_api():
            return openapi.Schema(
                ...
                properties={
                    ...
                    "star_num": openapi.Schema(
                        type=openapi.TYPE_INTEGER,
                        title="点赞数量",
                        description="点赞数量",
                        default=1,
                    ),
                    "trample_num": openapi.Schema(
                        type=openapi.TYPE_INTEGER,
                        title="点踩数量",
                        description="点踩数",
                        default=1,
