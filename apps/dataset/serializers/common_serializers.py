...
def update_document_char_length(document_id: str):
    update_execute(
        get_file_content(
            os.path.join(
                PROJECT_DIR, "apps", "dataset", "sql", "update_document_char_length.sql"
            )
        ),
...
class ProblemParagraphObject:
    def __init__(
        self, dataset_id: str, document_id: str, paragraph_id: str, problem_content: str
    ):
...
class ProblemParagraphManage:
    def __init__(
        self, problemParagraphObjectList: [ProblemParagraphObject], dataset_id
    ):
        self.dataset_id = dataset_id
    ...
    def to_problem_model_list(self):
        problem_list = [
            item.problem_content for item in self.problemParagraphObjectList
        ]
        exists_problem_list = []
        if len(self.problemParagraphObjectList) > 0:
            ...
            exists_problem_list = (
                QuerySet(Problem)
                .filter(dataset_id=self.dataset_id, content__in=problem_list)
                .all()
            )
        problem_content_dict = {}
        problem_model_list = [
            or_get(
                ...
                problemParagraphObject.dataset_id,
                problemParagraphObject.document_id,
                problemParagraphObject.paragraph_id,
                problem_content_dict,
            )
            for problemParagraphObject in self.problemParagraphObjectList
        ]
...
def get_embedding_model_by_dataset_id_list(dataset_id_list: List):
    ...
    return ModelManage.get_model(
        str(dataset_list[0].embedding_mode_id),
...
def get_embedding_model_by_dataset_id(dataset_id: str):
    dataset = (
        QuerySet(DataSet).select_related("embedding_mode").filter(id=dataset_id).first()
    )
    return ModelManage.get_model(
        str(dataset.embedding_mode_id), lambda _id: get_model(dataset.embedding_mode)
    )
