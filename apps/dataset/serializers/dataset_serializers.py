...
from dataset.models.data_set import (
    DataSet,
    Document,
    Paragraph,
    Problem,
...
class DataSetSerializers(serializers.ModelSerializer):
    ...
    class Application(ApiMixin, serializers.Serializer):
        ...
        dataset_id = serializers.UUIDField(
            required=True, error_messages=ErrMessage.char("数据集id")
        )
        ...
        def get_response_body_api():
            return openapi.Schema(
                ...
                properties={
                    "id": openapi.Schema(
                        type=openapi.TYPE_STRING, title="", description="主键id"
                    ),
                    "name": openapi.Schema(
                        type=openapi.TYPE_STRING,
                        title="应用名称",
                    ...
                    "model_id": openapi.Schema(
                        type=openapi.TYPE_STRING, title="模型id", description="模型id"
                    ),
                    "multiple_rounds_dialogue": openapi.Schema(
                        type=openapi.TYPE_BOOLEAN,
                        title="是否开启多轮对话",
                        description="是否开启多轮对话",
                    ),
                    "prologue": openapi.Schema(
                        type=openapi.TYPE_STRING, title="开场白", description="开场白"
                    ),
    ...
    class Query(ApiMixin, serializers.Serializer):
        ...
        def get_query_set(self):
            ...
            query_set_dict["dataset_custom_sql"] = QuerySet(
                model=get_dynamics_model(
                    {
        ...
        def page(self, current_page: int, page_size: int):
            return native_page_search(
                ...
                select_string=get_file_content(
                    os.path.join(
                        PROJECT_DIR, "apps", "dataset", "sql", "list_dataset.sql"
                    )
                ),
    ...
    class Create(ApiMixin, serializers.Serializer):
        ...
        class CreateBaseSerializers(ApiMixin, serializers.Serializer):
            ...
            embedding_mode_id = serializers.UUIDField(
                required=True, error_messages=ErrMessage.uuid("向量模型")
            )
        ...
        class CreateQASerializers(serializers.Serializer):
            ...
            file_list = serializers.ListSerializer(
                ...
                child=serializers.FileField(
                    required=True, error_messages=ErrMessage.file("文件")
                ),
            )
            ...
            def get_response_body_api():
                return openapi.Schema(
                    ...
                    properties={
                        ...
                        "char_length": openapi.Schema(
                            type=openapi.TYPE_STRING,
                            title="字符数",
                            description="字符数",
                            default=10,
                        ),
                        "document_count": openapi.Schema(
                            type=openapi.TYPE_STRING,
                            title="文档数量",
                            description="文档数量",
                            default=1,
        ...
        class CreateWebSerializers(serializers.Serializer):
            ...
            source_url = serializers.CharField(
                required=True,
                error_messages=ErrMessage.char("Web 根地址"),
            )
            ...
            def get_request_body_api():
                return openapi.Schema(
                    ...
                    properties={
                        ...
                        "embedding_mode_id": openapi.Schema(
                            type=openapi.TYPE_STRING,
                            title="向量模型id",
                            description="向量模型id",
                        ),
        ...
        def save_qa(self, instance: Dict, with_valid=True):
            ...
            document_list = flat_map(
                [DocumentSerializers.Create.parse_qa_file(file) for file in file_list]
            )
            dataset_instance = {
        ...
        def save(self, instance: Dict, with_valid=True):
            ...
            if (
                QuerySet(DataSet)
                .filter(user_id=user_id, name=instance.get("name"))
                .exists()
            ):
            ...
            problem_model_list, problem_paragraph_mapping_list = ProblemParagraphManage(
                problem_paragraph_object_list, dataset_id
            ...
            # 响应数据
            return {
                **DataSetSerializers(dataset).data,
                "document_list": DocumentSerializers.Query(
        ...
        def save_web(self, instance: Dict, with_valid=True):
            ...
            dataset.save()
            sync_web_dataset.delay(
                str(dataset_id), instance.get("source_url"), instance.get("selector")
            )
            return {**DataSetSerializers(dataset).data, "document_list": []}
    ...
    class Edit(serializers.Serializer):
        ...
        application_id_list = serializers.ListSerializer(
            ...
            child=serializers.UUIDField(
                required=True, error_messages=ErrMessage.char("应用id")
            ),
    ...
    class HitTest(ApiMixin, serializers.Serializer):
        ...
        user_id = serializers.UUIDField(
            required=False, error_messages=ErrMessage.char("用户id")
        )
        ...
        def hit_test(self):
            ...
            exclude_document_id_list = [
                ...
                for document in QuerySet(Document).filter(
                    dataset_id=self.data.get("id"), is_active=False
                )
            ]
    ...
    class SyncWeb(ApiMixin, serializers.Serializer):
        id = serializers.CharField(
            required=True, error_messages=ErrMessage.char("知识库id")
        )
        user_id = serializers.UUIDField(
            required=False, error_messages=ErrMessage.char("用户id")
        )
        sync_type = serializers.CharField(
            required=True,
            error_messages=ErrMessage.char("同步类型"),
            validators=[
                validators.RegexValidator(
            ...
            def handler(child_link: ChildLink, response: Fork.Response):
                if response.status == 200:
                    try:
                        ...
                        first = (
                            QuerySet(Document)
                            .filter(meta__source_url=child_link.url, dataset=dataset)
                            .first()
                        )
        ...
        def replace_sync(self, dataset):
            ...
            selector = (
                dataset.meta.get("selector") if "selector" in dataset.meta else None
            )
    ...
    class Operate(ApiMixin, serializers.Serializer):
        ...
        def export_excel(self, with_valid=True):
            ...
            paragraph_list = native_search(
                QuerySet(Paragraph).filter(dataset_id=self.data.get("id")),
                get_file_content(
                    os.path.join(
                        PROJECT_DIR,
                        "apps",
        ...
        def re_embedding(self, with_valid=True):
            ...
            QuerySet(Document).filter(dataset_id=self.data.get("id")).update(
                **{"status": Status.queue_up}
            )
            QuerySet(Paragraph).filter(dataset_id=self.data.get("id")).update(
                **{"status": Status.queue_up}
            )
            ...
            return select_list(
                get_file_content(
                    os.path.join(
                        ...
                        "dataset",
                        "sql",
                        "list_dataset_application.sql",
                    )
                ),
                [
                    (
                        self.data.get("user_id")
                        if self.data.get("user_id") == str(dataset.user_id)
                        else None
                    ),
                    dataset.user_id,
                    self.data.get("user_id"),
        ...
        def one(self, user_id, with_valid=True):
            ...
            query_set_dict = {
                ...
                ).filter(**{"temp.id": self.data.get("id")}),
                "dataset_custom_sql": QuerySet(
                    model=get_dynamics_model({"dataset.user_id": models.CharField()})
                ).filter(**{"dataset.user_id": user_id}),
                ...
                **native_search(
                ...
                "application_id_list": list(
                    filter(
                        ...
                        [
                            str(application_dataset_mapping.application_id)
                            for application_dataset_mapping in QuerySet(
                                ApplicationDatasetMapping
                            ).filter(dataset_id=self.data.get("id"))
                        ],
                    )
                ),
            }
        ...
        def edit(self, dataset: Dict, user_id: str):
            ...
            if (
                QuerySet(DataSet)
                .filter(user_id=user_id, name=dataset.get("name"))
                .exclude(id=self.data.get("id"))
                .exists()
            ):
                raise AppApiException(500, "知识库名称重复!")
            ...
            if (
                "application_id_list" in dataset
                and dataset.get("application_id_list") is not None
            ):
                application_id_list = dataset.get("application_id_list")
                # 当前用户可修改关联的知识库列表
                application_dataset_id_list = [
                    str(dataset_dict.get("id"))
                    for dataset_dict in self.list_application(with_valid=False)
                ]
                for dataset_id in application_id_list:
                    if not application_dataset_id_list.__contains__(dataset_id):
                        raise AppApiException(
                            500, f"未知的应用id${dataset_id},无法关联"
                        )

                # 删除已经关联的id
                QuerySet(ApplicationDatasetMapping).filter(
                    application_id__in=application_dataset_id_list,
                    dataset_id=self.data.get("id"),
                ).delete()
                # 插入
                (
                    ...
                    if len(application_id_list) > 0
                    else None
                )
                [
                    ApplicationDatasetMapping(
                        application_id=application_id, dataset_id=self.data.get("id")
                    )
                    for application_id in application_id_list
                ]
        ...
        def get_request_body_api():
            return openapi.Schema(
                ...
                properties={
                    ...
                    "application_id_list": openapi.Schema(
                        type=openapi.TYPE_ARRAY,
                        title="应用id列表",
                        description="应用id列表",
                        items=openapi.Schema(type=openapi.TYPE_STRING),
