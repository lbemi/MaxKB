...
class PGVector(BaseVectorStore):
    ...
    def hit_test(
        ...
        query_set = QuerySet(Embedding).filter(
            dataset_id__in=dataset_id_list, is_active=True
        )
    ...
    def query(
        ...
        if dataset_id_list is None or len(dataset_id_list) == 0:
            return []
        query_set = QuerySet(Embedding).filter(
            dataset_id__in=dataset_id_list, is_active=is_active
        )
        if exclude_document_id_list is not None and len(exclude_document_id_list) > 0:
    ...
    def handle(
        ...
        exec_sql, exec_params = generate_sql_by_query_dict(
            ...
            select_string=get_file_content(
                os.path.join(
                    PROJECT_DIR, "apps", "embedding", "sql", "embedding_search.sql"
                )
            ),
...
class KeywordsSearch(ISearch):
    def handle(
        ...
        exec_sql, exec_params = generate_sql_by_query_dict(
            ...
            select_string=get_file_content(
                os.path.join(
                    PROJECT_DIR, "apps", "embedding", "sql", "keywords_search.sql"
                )
            ),
            with_table_name=True,
        )
...
class BlendSearch(ISearch):
    def handle(
        ...
        exec_sql, exec_params = generate_sql_by_query_dict(
            ...
            select_string=get_file_content(
                os.path.join(
                    PROJECT_DIR, "apps", "embedding", "sql", "blend_search.sql"
                )
            ),
            with_table_name=True,
        )
