...
def update_embedding_search_vector(embedding, paragraph_list):
    paragraphs = [
        ...
        if paragraph.id == embedding.get("paragraph")
    ]
    if len(paragraphs) > 0:
...
def save_keywords(apps, schema_editor):
    try:
        ...
        for document in document_list:
            ...
            document.save()
            paragraph_list = (
                paragraph.objects.using(db_alias).filter(document=document).all()
            )
            embedding_list = (
                embedding.objects.using(db_alias)
                .filter(document=document)
                .values("id", "search_vector", "paragraph")
            )
            embedding_update_list = [
                update_embedding_search_vector(embedding, paragraph_list)
