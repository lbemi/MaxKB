...
class ListenerManagement:
    ...
    def embedding_by_paragraph_list(paragraph_id_list, embedding_model: Embeddings):
        try:
            ...
            max_kb_error.error(
                f"查询向量数据:{paragraph_id_list}出现错误{str(e)}{traceback.format_exc()}"
            )
    ...
    def embedding_by_paragraph_data_list(
        ...
        try:
            ...
            max_kb_error.error(
                f"向量化段落:{paragraph_id_list}出现错误{str(e)}{traceback.format_exc()}"
            )
            status = Status.error
        finally:
            QuerySet(Paragraph).filter(id__in=paragraph_id_list).update(
                **{"status": status}
            )
    ...
    def embedding_by_paragraph(paragraph_id, embedding_model: Embeddings):
        ...
        try:
            ...
            max_kb_error.error(
                f"向量化段落:{paragraph_id}出现错误{str(e)}{traceback.format_exc()}"
            )
            status = Status.error
        finally:
    ...
    def embedding_by_document(document_id, embedding_model: Embeddings):
        ...
        try:
            data_list = native_search(
                {
                    "problem": QuerySet(
                        get_dynamics_model(
                            {"paragraph.document_id": django.db.models.CharField()}
                        )
            ...
            max_kb_error.error(
                f"向量化文档:{document_id}出现错误{str(e)}{traceback.format_exc()}"
            )
            status = Status.error
        finally:
            # 修改状态
    ...
    def embedding_by_dataset(dataset_id, embedding_model: Embeddings):
        ...
        try:
            ...
            max_kb_error.error(
                f"向量化数据集:{dataset_id}出现错误{str(e)}{traceback.format_exc()}"
            )
        finally:
    ...
    def update_embedding_dataset_id(args: UpdateEmbeddingDatasetIdArgs):
        VectorStore.get_embedding_vector().update_by_paragraph_ids(
            args.paragraph_id_list, {"dataset_id": args.target_dataset_id}
        )
