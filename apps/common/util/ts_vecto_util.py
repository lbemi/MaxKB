...
def to_ts_vector(text: str):
    ...
    # 过滤标点符号
    result = [
        item for item in result if filter_word.__contains__(item) and len(item) < 10
    ]
    result_ = [
        {"word": get_key_by_word_dict(result[index], word_dict), "index": index}
        for index in range(len(result))
    ]
    result_group = group_by(result_, lambda r: r["word"])
    return " ".join(
        [
            f"{key.lower()}:{','.join([str(item['index'] + 1) for item in result_group[key]][:20])}"
            for key in result_group
            if not remove_chars.__contains__(key) and len(key.strip()) >= 0
        ]
    )
...
def to_query(text: str):
    ...
    extract_tags = analyse.extract_tags(
        text, topK=5, withWeight=True, allowPOS=("ns", "n", "vn", "v", "eng")
    )
    result = " ".join(
        [
            get_key_by_word_dict(word, word_dict)
            for word, score in extract_tags
            if not remove_chars.__contains__(word)
        ]
    )
    # 删除词库
    for word in word_list:
