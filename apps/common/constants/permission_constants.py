...
class RoleConstants(Enum):
    ...
    APPLICATION_ACCESS_TOKEN = (
        Role("会话", "只拥有应用会话框接口权限", RoleGroup.APPLICATION_ACCESS_TOKEN),
    )
...
class PermissionConstants(Enum):
    ...
    USER_DELETE = Permission(
        group=Group.USER, operate=Operate.DELETE, roles=[RoleConstants.USER]
    )

    DATASET_CREATE = Permission(
        group=Group.DATASET,
        operate=Operate.CREATE,
        roles=[RoleConstants.ADMIN, RoleConstants.USER],
    )

    DATASET_READ = Permission(
        group=Group.DATASET,
        operate=Operate.READ,
        roles=[RoleConstants.ADMIN, RoleConstants.USER],
    )
    ...
    TEAM_READ = Permission(
        group=Group.TEAM,
        operate=Operate.READ,
        roles=[RoleConstants.ADMIN, RoleConstants.USER],
    )

    TEAM_CREATE = Permission(
        group=Group.TEAM,
        operate=Operate.CREATE,
        roles=[RoleConstants.ADMIN, RoleConstants.USER],
    )

    TEAM_DELETE = Permission(
        group=Group.TEAM,
        operate=Operate.DELETE,
        roles=[RoleConstants.ADMIN, RoleConstants.USER],
    )

    TEAM_EDIT = Permission(
        group=Group.TEAM,
        operate=Operate.EDIT,
        roles=[RoleConstants.ADMIN, RoleConstants.USER],
    )
...
def get_permission_list_by_role(role: RoleConstants):
    ...
    return list(
        map(
            ...
            list(
                filter(
                    lambda k: PermissionConstants[k].value.roleList.__contains__(role),
