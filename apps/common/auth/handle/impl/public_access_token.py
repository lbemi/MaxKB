...
class PublicAccessToken(AuthBaseHandle):
    ...
    def handle(self, request, token: str, get_token_details):
        ...
        application_access_token = (
            ...
            .filter(application_id=auth_details.get("application_id"))
            .first()
        )
        ...
        if not application_access_token.is_active:
            raise AppAuthenticationFailed(1002, "身份验证信息不正确")
        if not application_access_token.access_token == auth_details.get(
            "access_token"
        ):
            raise AppAuthenticationFailed(1002, "身份验证信息不正确")
        ...
        return application_access_token.application.user, Auth(
            ...
            permission_list=[
                Permission(
                    ...
                    operate=Operate.USE,
                    dynamic_tag=str(application_access_token.application_id),
                )
            ],
