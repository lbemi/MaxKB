...
class LoginSerializer(ApiMixin, serializers.Serializer):
    username = serializers.CharField(
        required=True, error_messages=ErrMessage.char("用户名")
    )
        ...
        return openapi.Schema(
            ...
            properties={
                ...
                "password": openapi.Schema(
                    type=openapi.TYPE_STRING, title="密码", description="密码"
                ),
            },
        )
...
class RegisterSerializer(ApiMixin, serializers.Serializer):
    ...
    username = serializers.CharField(
        ...
        validators=[
            validators.RegexValidator(
                regex=re.compile("^[a-zA-Z][a-zA-Z0-9_]{5,20}$"),
    ...
    password = serializers.CharField(
        required=True,
        error_messages=ErrMessage.char("密码"),
        validators=[
            validators.RegexValidator(
                regex=re.compile(
    ...
    @lock(
        lock_key=lambda this, raise_exception: (
        ...
        return openapi.Schema(
            ...
            properties={
                ...
                "re_password": openapi.Schema(
                    type=openapi.TYPE_STRING, title="确认密码", description="确认密码"
                ),
                "code": openapi.Schema(
                    type=openapi.TYPE_STRING, title="验证码", description="验证码"
                ),
            },
        )
...
class SendEmailSerializer(ApiMixin, serializers.Serializer):
    ...
    def send(self):
        ...
        file = open(
            os.path.join(
                PROJECT_DIR, "apps", "common", "template", "email_template.html"
            ),
            "r",
            encoding="utf-8",
        )
        ...
        try:
            ...
            # 发送邮件
            send_mail(
                f'【MaxKB 智能知识库-{"用户注册" if state == "register" else "修改密码"}】',
                "",
    ...
    class Query(ApiMixin):
        ...
        def list(self, with_valid=True):
            ...
            return [
                ...
                for user_model in QuerySet(User).filter(
        ...
        return openapi.Schema(
            ...
            properties={
                ...
                "phone": openapi.Schema(
                    type=openapi.TYPE_STRING, title="手机号", description="手机号"
                ),
                "is_active": openapi.Schema(
                    type=openapi.TYPE_BOOLEAN, title="是否激活", description="是否激活"
                ),
                "role": openapi.Schema(
                    type=openapi.TYPE_STRING, title="角色", description="角色"
                ),
                "source": openapi.Schema(
                    type=openapi.TYPE_STRING, title="来源", description="来源"
                ),
                "nick_name": openapi.Schema(
                    type=openapi.TYPE_STRING, title="姓名", description="姓名"
                ),
                "create_time": openapi.Schema(
                    type=openapi.TYPE_STRING, title="创建时间", description="修改时间"
                ),
                "update_time": openapi.Schema(
                    type=openapi.TYPE_STRING, title="修改时间", description="修改时间"
                ),
            },
        )
...
class UserManageSerializer(serializers.Serializer):
        ...
        def get_query_set(self):
            ...
            if email_or_username is not None:
                query_set = query_set.filter(
                    Q(username__contains=email_or_username)
                    | Q(email__contains=email_or_username)
                )
    ...
    class UserInstance(ApiMixin, serializers.Serializer):
        ...
        nick_name = serializers.CharField(
            required=False,
            error_messages=ErrMessage.char("姓名"),
            max_length=64,
            allow_null=True,
            allow_blank=True,
        )
        phone = serializers.CharField(
            required=False,
            error_messages=ErrMessage.char("手机号"),
            max_length=20,
            allow_null=True,
            allow_blank=True,
        )
    ...
    class UserEditInstance(ApiMixin, serializers.Serializer):
        ...
        def is_valid(self, *, user_id=None, raise_exception=False):
            super().is_valid(raise_exception=True)
            if (
                self.data.get("email") is not None
                and QuerySet(User)
        ...
        def get_request_body_api():
            return openapi.Schema(
                ...
                properties={
                    "email": openapi.Schema(
                        type=openapi.TYPE_STRING, title="邮箱", description="邮箱"
                    ),
                    "nick_name": openapi.Schema(
                        type=openapi.TYPE_STRING, title="姓名", description="姓名"
                    ),
    ...
    def save(self, instance, with_valid=True):
        if with_valid:
            UserManageSerializer.UserInstance(data=instance).is_valid(
        ...
        user = User(
            ...
            nick_name=(
                "" if instance.get("nick_name") is None else instance.get("nick_name")
            ),
    ...
    class Operate(serializers.Serializer):
        ...
        def delete(self, with_valid=True):
            ...
            QuerySet(TeamMemberPermission).filter(
                member_id__in=[team_member.id for team_member in team_member_list]
            ).delete()
            # 删除团队成员
