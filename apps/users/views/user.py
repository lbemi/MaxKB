    ...
    class Query(APIView):
        ...
        @swagger_auto_schema(
            ...
            responses=result.get_api_array_response(
                UserSerializer.Query.get_response_body_api()
            ),
            tags=["用户"],
        )
        @has_permissions(PermissionConstants.USER_READ)
        def get(self, request: Request):
            return result.success(
                UserSerializer.Query(
                    data={
                        "email_or_username": request.query_params.get(
                            "email_or_username"
                        )
                    }
                ).list()
