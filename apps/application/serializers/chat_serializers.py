...
class WorkFlowSerializers(serializers.Serializer):
    nodes = serializers.ListSerializer(
        child=serializers.DictField(), error_messages=ErrMessage.uuid("节点")
    )
    edges = serializers.ListSerializer(
        child=serializers.DictField(), error_messages=ErrMessage.uuid("连线")
    )
...
class ChatSerializers(serializers.Serializer):
    class Operate(serializers.Serializer):
        chat_id = serializers.UUIDField(
            required=True, error_messages=ErrMessage.uuid("对话id")
        )
    ...
    class ClientChatHistory(serializers.Serializer):
        ...
        client_id = serializers.UUIDField(
            required=True, error_messages=ErrMessage.uuid("客户端id")
        )
        ...
        def page(self, current_page: int, page_size: int, with_valid=True):
            ...
            return page_search(
                ...
                page_size,
                queryset,
                lambda row: ChatSerializerModel(row).data,
            )

    class Query(serializers.Serializer):
        abstract = serializers.CharField(
            required=False, error_messages=ErrMessage.char("摘要")
        )
        history_day = serializers.IntegerField(
            required=True, error_messages=ErrMessage.integer("历史天数")
        )
        ...
        min_star = serializers.IntegerField(
            required=False, min_value=0, error_messages=ErrMessage.integer("最小点赞数")
        )
        min_trample = serializers.IntegerField(
            required=False, min_value=0, error_messages=ErrMessage.integer("最小点踩数")
        )
        comparer = serializers.CharField(
            required=False,
            error_messages=ErrMessage.char("比较器"),
            validators=[
                validators.RegexValidator(
                    regex=re.compile("^and|or$"), message="只支持and|or", code=500
                )
            ],
        )
        ...
        def to_row(row: Dict):
            ...
            padding_problem_text = (
                details.get("problem_padding").get("padding_problem_text")
                if "problem_padding" in details
                and "padding_problem_text" in details.get("problem_padding")
                else ""
            )
            paragraph_list = (
                details.get("search_step").get("paragraph_list")
                if "search_step" in details
                and "paragraph_list" in details.get("search_step")
                else []
            )
            ...
            return [
                ...
                row.get("message_tokens") + row.get("answer_tokens"),
                row.get("run_time"),
    ...
    class OpenChat(serializers.Serializer):
        ...
        def is_valid(self, *, raise_exception=False):
            ...
            if (
                ...
                .filter(id=application_id, user_id=user_id)
                .exists()
            ):
        ...
        def open_work_flow(self, application):
            ...
            chat_cache.set(
                chat_id,
                ChatInfo(chat_id, [], [], application, work_flow_version),
                timeout=60 * 30,
            )
            return chat_id
        ...
        def open_simple(self, application):
            valid_model_params_setting(
                application.model_id, application.model_params_setting
            )
            application_id = self.data.get("application_id")
            dataset_id_list = [
                str(row.dataset_id)
                for row in QuerySet(ApplicationDatasetMapping).filter(
    ...
    class OpenTempChat(serializers.Serializer):
        ...
        multiple_rounds_dialogue = serializers.BooleanField(
            required=True, error_messages=ErrMessage.boolean("多轮会话")
        )
        ...
        model_params_setting = serializers.JSONField(
            required=False, error_messages=ErrMessage.dict("模型参数相关设置")
        )
        ...
        def get_user_id(self):
            if "id" in self.data and self.data.get("id") is not None:
                application = (
                    QuerySet(Application).filter(id=self.data.get("id")).first()
                )
        ...
        def open(self):
            ...
            dataset_id_list = self.data.get("dataset_id_list")
            dialogue_number = (
                3 if self.data.get("multiple_rounds_dialogue", False) else 0
            )
...
class ChatRecordSerializer(serializers.Serializer):
    class Operate(serializers.Serializer):
        ...
        chat_record_id = serializers.UUIDField(
            required=True, error_messages=ErrMessage.uuid("对话记录id")
        )
        ...
        def is_valid(self, *, current_role=None, raise_exception=False):
            ...
            if (
                not application_access_token.show_source
                and current_role == RoleConstants.APPLICATION_ACCESS_TOKEN.value
            ):
    ...
    class Query(serializers.Serializer):
        ...
        def reset_chat_record(chat_record):
            ...
            if (
                ...
                and chat_record.details.get("search_step").get("paragraph_list")
                is not None
            ):
                ...
                dataset_list = [
                    ...
                    for dataset_id, name in reduce(
                        lambda x, y: {**x, **y},
                        [
                            {row.get("dataset_id"): row.get("dataset_name")}
            ...
            return {
                **ChatRecordSerializerModel(chat_record).data,
                "padding_problem_text": (
                    chat_record.details.get("problem_padding").get(
                        "padding_problem_text"
                    )
                ...
                "execution_details": [
                    chat_record.details[key] for key in chat_record.details
                ],
            }
    ...
    class Vote(serializers.Serializer):
        ...
        vote_status = serializers.ChoiceField(
            choices=VoteChoices.choices, error_messages=ErrMessage.uuid("投标状态")
        )
    ...
    class ImproveSerializer(serializers.Serializer):
        ...
        content = serializers.CharField(
            required=True, error_messages=ErrMessage.char("段落内容")
        )
    ...
    class ChatRecordImprove(serializers.Serializer):
        ...
        def get(self, with_valid=True):
            ...
            if len(paragraph_model_list) < len(chat_record.improve_paragraph_id_list):
                ...
                chat_record.save()
            return [
                ChatRecordSerializer.ParagraphModel(p).data
                for p in paragraph_model_list
            ]
    ...
    class Improve(serializers.Serializer):
        ...
        document_id = serializers.UUIDField(
            required=True, error_messages=ErrMessage.uuid("文档id")
        )
        ...
        def improve(self, instance: Dict, with_valid=True):
            if with_valid:
                self.is_valid(raise_exception=True)
            ChatRecordSerializer.ImproveSerializer(data=instance).is_valid(
            ...
            problem = Problem(
                id=uuid.uuid1(), content=problem_text, dataset_id=dataset_id
            )
        ...
        class Operate(serializers.Serializer):
            ...
            paragraph_id = serializers.UUIDField(
                required=True, error_messages=ErrMessage.uuid("段落id")
            )
            ...
            def delete(self, with_valid=True):
                ...
                if not chat_record.improve_paragraph_id_list.__contains__(
                    ...
                    raise AppApiException(
                        500, f"段落id错误,当前对话记录不存在【{paragraph_id}】段落id"
                    )
                chat_record.improve_paragraph_id_list = [
                    row
