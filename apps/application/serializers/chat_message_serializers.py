...
from application.models.api_key_model import (
...
class ChatInfo:
    ...
    def to_base_pipeline_manage_params(self):
        ...
        model_id = (
            self.application.model.id if self.application.model is not None else None
        )
        ...
        return {
            ...
            "similarity": (
                dataset_setting.get("similarity")
                if "similarity" in dataset_setting
                else 0.6
            ),
            "max_paragraph_char_number": (
                dataset_setting.get("max_paragraph_char_number")
                if "max_paragraph_char_number" in dataset_setting
                else 5000
            ),
            ...
            "search_mode": (
                self.application.dataset_setting.get("search_mode")
                if "search_mode" in self.application.dataset_setting
                else "embedding"
            ),
            "no_references_setting": (
                self.application.dataset_setting.get("no_references_setting")
                if "no_references_setting" in self.application.dataset_setting
                else {
    ...
    def append_chat_record(self, chat_record: ChatRecord, client_id=None):
        chat_record.problem_text = (
            chat_record.problem_text[0:1024]
        ...
        if self.application.id is not None:
            ...
            if not QuerySet(Chat).filter(id=self.chat_id).exists():
                Chat(
                    ...
                    abstract=chat_record.problem_text,
                    client_id=client_id,
                ).save()
            # 插入会话记录
            chat_record.save()
...
class ChatMessageSerializer(serializers.Serializer):
    ...
    def is_valid_intraday_access_num(self):
        if (
            ...
            access_client = (
                ...
                .filter(id=self.data.get("client_id"))
                .first()
            )
            if access_client is None:
                access_client = ApplicationPublicAccessClient(
    ...
    def chat_work_flow(self, chat_info: ChatInfo):
        ...
        work_flow_manage = WorkflowManage(
            Flow.new_instance(chat_info.work_flow_version.work_flow),
            {
                "history_chat_record": chat_info.chat_record_list,
                "question": message,
                "chat_id": chat_info.chat_id,
                "chat_record_id": str(uuid.uuid1()),
                "stream": stream,
                "re_chat": re_chat,
                "user_id": user_id,
            },
            WorkFlowPostHandler(chat_info, client_id, client_type),
        )
