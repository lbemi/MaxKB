...
from dataset.serializers.common_serializers import (
    list_paragraph,
    get_embedding_model_by_dataset_id_list,
)
...
class ModelDatasetAssociation(serializers.Serializer):
    ...
    dataset_id_list = serializers.ListSerializer(
        ...
        child=serializers.UUIDField(
            required=True, error_messages=ErrMessage.uuid("知识库id")
        ),
    ...
    def is_valid(self, *, raise_exception=True):
        ...
        exist_dataset_id_list = [
            str(dataset.id)
            for dataset in QuerySet(DataSet).filter(
                id__in=dataset_id_list, user_id=user_id
            )
        ]
...
class NoReferencesSetting(serializers.Serializer):
    ...
    value = serializers.CharField(
        required=True, error_messages=ErrMessage.char("提示词")
    )
...
def valid_model_params_setting(model_id, model_params_setting):
    if (
        model_id is None
        or model_params_setting is None
        or len(model_params_setting.keys()) == 0
    ):
        return
    ...
    credential.get_model_params_setting_form(model.model_name).valid_form(
...
class DatasetSettingSerializer(serializers.Serializer):
    ...
    search_mode = serializers.CharField(
        ...
        validators=[
            validators.RegexValidator(
                ...
                message="类型只支持register|reset_password",
                code=500,
            )
        ],
    ...
    no_references_setting = NoReferencesSetting(
        required=True, error_messages=ErrMessage.base("未引用分段设置")
    )


class ModelSettingSerializer(serializers.Serializer):
    prompt = serializers.CharField(
        required=True, max_length=2048, error_messages=ErrMessage.char("提示词")
    )
...
class ApplicationWorkflowSerializer(serializers.Serializer):
    ...
    work_flow = serializers.DictField(
        required=False, error_messages=ErrMessage.dict("工作流对象")
    )
...
class ApplicationSerializer(serializers.Serializer):
    ...
    multiple_rounds_dialogue = serializers.BooleanField(
        required=True, error_messages=ErrMessage.char("多轮对话")
    )
    prologue = serializers.CharField(
        required=False,
        allow_null=True,
    ...
    problem_optimization = serializers.BooleanField(
        required=True, error_messages=ErrMessage.boolean("问题补全")
    )
    # 应用类型
    type = serializers.CharField(
        required=True,
        error_messages=ErrMessage.char("应用类型"),
        validators=[
            validators.RegexValidator(
                regex=re.compile("^SIMPLE|WORK_FLOW$"),
                message="应用类型只支持SIMPLE|WORK_FLOW",
                code=500,
            )
        ],
    )
    model_params_setting = serializers.DictField(
        required=False, error_messages=ErrMessage.dict("模型参数")
    )
    ...
    class Embed(serializers.Serializer):
        host = serializers.CharField(
            required=True, error_messages=ErrMessage.char("主机")
        )
        protocol = serializers.CharField(
            required=True, error_messages=ErrMessage.char("协议")
        )
        token = serializers.CharField(
            required=True, error_messages=ErrMessage.char("token")
        )
        ...
        def get_embed(self, with_valid=True):
            ...
            index_path = os.path.join(
                PROJECT_DIR, "apps", "application", "template", "embed.js"
            )
            ...
            show_guide = "true"
            float_icon = (
                f"{self.data.get('protocol')}://{self.data.get('host')}/ui/favicon.ico"
            )
    ...
    class AccessTokenSerializer(serializers.Serializer):
        application_id = serializers.UUIDField(
            required=True, error_messages=ErrMessage.boolean("应用id")
        )

        class AccessTokenEditSerializer(serializers.Serializer):
            access_token_reset = serializers.BooleanField(
                required=False, error_messages=ErrMessage.boolean("重置Token")
            )
            is_active = serializers.BooleanField(
                required=False, error_messages=ErrMessage.boolean("是否开启")
            )
            ...
            white_active = serializers.BooleanField(
                required=False, error_messages=ErrMessage.boolean("是否开启白名单")
            )
            white_list = (
                serializers.ListSerializer(
                    required=False,
                    child=serializers.CharField(
                        required=True, error_messages=ErrMessage.char("白名单")
                    ),
                    error_messages=ErrMessage.list("白名单列表"),
                ),
            )
            show_source = serializers.BooleanField(
                required=False, error_messages=ErrMessage.boolean("是否显示知识来源")
            )

        def edit(self, instance: Dict, with_valid=True):
            if with_valid:
                self.is_valid(raise_exception=True)
                ApplicationSerializer.AccessTokenSerializer.AccessTokenEditSerializer(
                    data=instance
        ...
        def one(self, with_valid=True):
            ...
            if application_access_token is None:
                application_access_token = ApplicationAccessToken(
                    ...
                    access_token=hashlib.md5(str(uuid.uuid1()).encode()).hexdigest()[
                        8:24
                    ],
                    is_active=True,
                )
    ...
    class Authentication(serializers.Serializer):
        access_token = serializers.CharField(
            required=True, error_messages=ErrMessage.char("access_token")
        )
        ...
        def auth(self, request, with_valid=True):
            ...
            if (
                ...
                if (
                    token_details is not None
                    and "client_id" in token_details
                    and token_details.get("client_id") is not None
                ):
    ...
    class Edit(serializers.Serializer):
        ...
        multiple_rounds_dialogue = serializers.BooleanField(
            required=False, error_messages=ErrMessage.boolean("多轮会话")
        )
        prologue = serializers.CharField(
            required=False,
            allow_null=True,
        ...
        model_setting = ModelSettingSerializer(
            required=False, allow_null=True, error_messages=ErrMessage.json("模型设置")
        )
        # 问题补全
        problem_optimization = serializers.BooleanField(
            required=False,
        ...
        icon = serializers.CharField(
            required=False, allow_null=True, error_messages=ErrMessage.char("icon图标")
        )
    ...
    class Create(serializers.Serializer):
        ...
        def insert_workflow(self, application: Dict):
            self.is_valid(raise_exception=True)
            user_id = self.data.get("user_id")
            ApplicationWorkflowSerializer(data=application).is_valid(
                raise_exception=True
            )
            application_model = ApplicationWorkflowSerializer.to_application_model(
                user_id, application
            )
        ...
        def insert_simple(self, application: Dict):
            self.is_valid(raise_exception=True)
            user_id = self.data.get("user_id")
            ApplicationSerializer(data=application).is_valid(
                user_id=user_id, raise_exception=True
            )
            application_model = ApplicationSerializer.Create.to_application_model(
                user_id, application
            )
            dataset_id_list = application.get("dataset_id_list", [])
            application_dataset_mapping_model_list = [
                ApplicationSerializer.Create.to_application_dataset_mapping(
        ...
        def to_application_dataset_mapping(application_id: str, dataset_id: str):
            return ApplicationDatasetMapping(
                id=uuid.uuid1(), application_id=application_id, dataset_id=dataset_id
            )
    ...
    class HitTest(serializers.Serializer):
        ...
        def hit_test(self):
            ...
            dataset_id_list = [
                ad.dataset_id
                for ad in QuerySet(ApplicationDatasetMapping).filter(
    ...
    class Query(serializers.Serializer):
        name = serializers.CharField(
            required=False, error_messages=ErrMessage.char("应用名称")
        )

        desc = serializers.CharField(
            required=False, error_messages=ErrMessage.char("应用描述")
        )
        ...
        def get_query_set(self):
            ...
            query_set = QuerySet(
                model=get_dynamics_model(
                    {
                        "temp_application.name": models.CharField(),
                        "temp_application.desc": models.CharField(),
            ...
            if "desc" in self.data and self.data.get("desc") is not None:
                query_set = query_set.filter(
                    **{"temp_application.desc__icontains": self.data.get("desc")}
                )
            if "name" in self.data and self.data.get("name") is not None:
                query_set = query_set.filter(
                    **{"temp_application.name__icontains": self.data.get("name")}
                )
            query_set = query_set.order_by("-temp_application.create_time")
            query_set_dict["default_sql"] = query_set

            query_set_dict["application_custom_sql"] = QuerySet(
                model=get_dynamics_model(
                    {
            ...
            query_set_dict["team_member_permission_custom_sql"] = QuerySet(
                model=get_dynamics_model(
                    {
                ...
                **{
                    "user_id": user_id,
                    "team_member_permission.operate__contains": ["USE"],
        ...
        def list(self, with_valid=True):
            if with_valid:
                self.is_valid(raise_exception=True)
            return [
                ApplicationSerializer.Query.reset_application(a)
                for a in native_search(
                    self.get_query_set(),
        ...
        def reset_application(application: Dict):
            application["multiple_rounds_dialogue"] = (
                True if application.get("dialogue_number") > 0 else False
            )
            del application["dialogue_number"]
            if "dataset_setting" in application:
                application["dataset_setting"] = {
    ...
    class IconOperate(serializers.Serializer):
        ...
        image = UploadedImageField(
            required=True, error_messages=ErrMessage.image("图片")
        )
        ...
        def edit(self, with_valid=True):
            if with_valid:
                self.is_valid(raise_exception=True)
            application = (
                QuerySet(Application).filter(id=self.data.get("application_id")).first()
            )
            ...
            get_application_access_token(application_access_token.access_token, False)
            return {
                **ApplicationSerializer.Query.reset_application(
    ...
    class Operate(serializers.Serializer):
        ...
        model_id = serializers.UUIDField(
            required=False, error_messages=ErrMessage.uuid("模型id")
        )
        ai_node_id = serializers.UUIDField(
            required=False, error_messages=ErrMessage.uuid("AI节点id")
        )

        def is_valid(self, *, raise_exception=False):
            super().is_valid(raise_exception=True)
            if (
                not QuerySet(Application)
                .filter(id=self.data.get("application_id"))
                .exists()
            ):
                raise AppApiException(500, "不存在的应用id")
        ...
        def list_model(self, model_type=None, with_valid=True):
            ...
            return ModelSerializer.Query(
                data={"user_id": application.user_id, "model_type": model_type}
        ...
        def get_function_lib(self, function_lib_id, with_valid=True):
            ...
            return FunctionLibSerializer.Operate(
                data={"user_id": application.user_id, "id": function_lib_id}
            ).one(with_valid=True)
        ...
        def get_model_params_form(self, model_id, with_valid=True):
            ...
            return ModelSerializer.ModelParams(
                data={"user_id": application.user_id, "id": model_id}
            ).get_model_params(with_valid=True)
        ...
        def publish(self, instance, with_valid=True):
            ...
            application_dataset_id_list = [
                str(dataset.get("id")) for dataset in dataset_list
            ]
            dataset_id_list = self.update_reverse_search_node(
                work_flow, application_dataset_id_list
            )
            application.work_flow = work_flow
            application.save()
            # 插入知识库关联关系
            self.save_application_mapping(
                application_dataset_id_list, dataset_id_list, application.id
            )
            work_flow_version = WorkFlowVersion(
                work_flow=work_flow, application=application
            )
        ...
        def one(self, with_valid=True):
            ...
            dataset_list = self.list_dataset(with_valid=False)
            mapping_dataset_id_list = [
                adm.dataset_id
                for adm in QuerySet(ApplicationDatasetMapping).filter(
                    application_id=application_id
                )
            ]
            dataset_id_list = [
                ...
                for d in list(
                    filter(
                        lambda row: mapping_dataset_id_list.__contains__(row.get("id")),
                        dataset_list,
                    )
                )
            ]
            self.update_search_node(
        ...
        def update_search_node(self, work_flow, user_dataset_id_list: List):
            ...
            for search_node in search_node_list:
                ...
                node_data["dataset_id_list"] = [
                    dataset_id
                    for dataset_id in dataset_id_list
                    if user_dataset_id_list.__contains__(dataset_id)
                ]
                ...
                for dataset_id in dataset_id_list:
                    if not user_dataset_id_list.__contains__(dataset_id):
                        raise AppApiException(
                            500, f"未知的知识库id${dataset_id},无法关联"
                        )

                source_dataset_id_list = node_data.get("source_dataset_id_list", [])
                source_dataset_id_list = [
                    source_dataset_id
                    for source_dataset_id in source_dataset_id_list
                    if not user_dataset_id_list.__contains__(source_dataset_id)
                ]
                source_dataset_id_list = list(
                    {*source_dataset_id_list, *dataset_id_list}
                )
        ...
        def edit(self, instance: Dict, with_valid=True):
            ...
            application_id = self.data.get("application_id")
            valid_model_params_setting(
                instance.get("model_id"), instance.get("model_params_setting")
            )
            ...
            if "work_flow" in instance:
                # 当前用户可修改关联的知识库列表
                application_dataset_id_list = [
                    str(dataset_dict.get("id"))
                    for dataset_dict in self.list_dataset(with_valid=False)
                ]
                self.update_reverse_search_node(
                    instance.get("work_flow"), application_dataset_id_list
                )
                ...
                if update_key in instance and instance.get(update_key) is not None:
                    if update_key == "multiple_rounds_dialogue":
                        application.__setattr__(
                            "dialogue_number", 0 if not instance.get(update_key) else 3
                        )
                    else:
            ...
            if "dataset_id_list" in instance:
                ...
                self.save_application_mapping(
                    application_dataset_id_list, dataset_id_list, application_id
                )
        ...
        @staticmethod
        def save_application_mapping(
            application_dataset_id_list, dataset_id_list, application_id
        ):
            # 需要排除已删除的数据集
            dataset_id_list = [
                dataset.id
                for dataset in QuerySet(DataSet).filter(id__in=dataset_id_list)
            ]
            # 删除已经关联的id
            QuerySet(ApplicationDatasetMapping).filter(
                dataset_id__in=application_dataset_id_list,
                application_id=application_id,
            ).delete()
            # 插入
            (
                QuerySet(ApplicationDatasetMapping).bulk_create(
                    [
                        ApplicationDatasetMapping(
                            application_id=application_id, dataset_id=dataset_id
                        )
                        for dataset_id in dataset_id_list
                    ]
                )
            ...
            return select_list(
                get_file_content(
                    os.path.join(
                        ...
                        "application",
                        "sql",
                        "list_application_dataset.sql",
                    )
                ),
                [
                    (
                        self.data.get("user_id")
                        if self.data.get("user_id") == str(application.user_id)
                        else None
                    ),
                    application.user_id,
                    self.data.get("user_id"),
        ...
        def get_other_file_list(self):
            ...
            if model:
                res = (
                    ModelProvideConstants[model.provider]
                    .value.get_model_credential(model.model_type, model.model_name)
                    .get_other_fields(model.model_name)
                )
        ...
        def save_other_config(self, data):
            application = Application.objects.filter(
                id=self.initial_data.get("application_id")
            ).first()
            if not application:
                return
    ...
    class ApplicationKeySerializer(serializers.Serializer):
        ...
        def generate(self, with_valid=True):
            ...
            application = QuerySet(Application).filter(id=application_id).first()
            secret_key = (
                "application-" + hashlib.md5(str(uuid.uuid1()).encode()).hexdigest()
            )
            ...
            return ApplicationSerializer.ApplicationKeySerializerModel(
                application_api_key
            ).data
        ...
        def list(self, with_valid=True):
            ...
            application_id = self.data.get("application_id")
            return [
                ApplicationSerializer.ApplicationKeySerializerModel(
                    application_api_key
                ).data
                for application_api_key in QuerySet(ApplicationApiKey).filter(
                    application_id=application_id
                )
            ]
        ...
        class Edit(serializers.Serializer):
            is_active = serializers.BooleanField(
                required=False, error_messages=ErrMessage.boolean("是否可用")
            )

            allow_cross_domain = serializers.BooleanField(
                required=False, error_messages=ErrMessage.boolean("是否允许跨域")
            )

            cross_domain_list = serializers.ListSerializer(
                required=False,
                child=serializers.CharField(
                    required=True, error_messages=ErrMessage.char("跨域列表")
                ),
        ...
        class Operate(serializers.Serializer):
            ...
            api_key_id = serializers.CharField(
                required=True, error_messages=ErrMessage.char("ApiKeyid")
            )
            ...
            def edit(self, instance, with_valid=True):
                if with_valid:
                    self.is_valid(raise_exception=True)
                    ApplicationSerializer.ApplicationKeySerializer.Edit(
                        data=instance
                    ).is_valid(raise_exception=True)
                api_key_id = self.data.get("api_key_id")
                application_id = self.data.get("application_id")
