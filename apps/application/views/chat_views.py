...
from application.serializers.chat_serializers import (
    ChatSerializers,
    ChatRecordSerializer,
)
from application.swagger_api.chat_api import (
    ChatApi,
    VoteApi,
    ChatRecordApi,
    ImproveApi,
    ChatRecordImproveApi,
...
class ChatView(APIView):
    ...
    class Open(APIView):
        ...
        @action(methods=["GET"], detail=False)
        @swagger_auto_schema(
            operation_summary="获取会话id,根据应用id",
        ...
        def get(self, request: Request, application_id: str):
            return result.success(
                ChatSerializers.OpenChat(
                    data={"user_id": request.user.id, "application_id": application_id}
                ).open()
            )
    ...
    class OpenWorkFlowTemp(APIView):
        ...
        def post(self, request: Request):
            return result.success(
                ChatSerializers.OpenWorkFlowChat(
    ...
    class OpenTemp(APIView):
        ...
        @action(methods=["POST"], detail=False)
        @swagger_auto_schema(
            operation_summary="获取会话id(根据模型id,知识库列表,是否多轮会话)",
            operation_id="获取会话id",
        ...
        def post(self, request: Request, chat_id: str):
            return ChatMessageSerializer(
                data={
                    ...
                    "re_chat": (
                        request.data.get("re_chat")
                        if "re_chat" in request.data
                        else False
                    ),
                    "stream": (
                        request.data.get("stream") if "stream" in request.data else True
                    ),
                    "application_id": (
                        request.auth.keywords.get("application_id")
                        if request.auth.client_type
                        == AuthenticationType.APPLICATION_ACCESS_TOKEN.value
                        else None
                    ),
    ...
    @has_permissions(
        ViewPermission(
            [RoleConstants.ADMIN, RoleConstants.USER, RoleConstants.APPLICATION_KEY],
            [
                lambda r, keywords: Permission(
                    group=Group.APPLICATION,
                    operate=Operate.USE,
    ...
    class ClientChatHistoryPage(APIView):
        ...
        def get(
            ...
            return result.success(
                ChatSerializers.ClientChatHistory(
                    data={
                        "client_id": request.auth.client_id,
                        "application_id": application_id,
                    }
    ...
    class Page(APIView):
        ...
        @action(methods=["GET"], detail=False)
        @swagger_auto_schema(
            operation_summary="分页获取对话列表",
            operation_id="分页获取对话列表",
            manual_parameters=result.get_page_request_params(
                ChatApi.get_request_params_api()
            ),
            responses=result.get_page_api_response(ChatApi.get_response_body_api()),
            tags=["应用/对话日志"],
        )
    ...
    class ChatRecord(APIView):
        ...
        class Operate(APIView):
            ...
            def get(
                ...
                return result.success(
                    ChatRecordSerializer.Operate(
                        data={
                            "application_id": application_id,
                            "chat_id": chat_id,
                            "chat_record_id": chat_record_id,
                        }
                    ).one(request.auth.current_role)
                )

        @action(methods=["GET"], detail=False)
        @swagger_auto_schema(
            operation_summary="获取对话记录列表",
        ...
        def get(self, request: Request, application_id: str, chat_id: str):
            return result.success(
                ChatRecordSerializer.Query(
                    data={
                        "application_id": application_id,
                        "chat_id": chat_id,
                        "order_asc": request.query_params.get("order_asc"),
                    }
                ).list()
            )
        ...
        class Vote(APIView):
            ...
            def put(
                ...
                return result.success(
                    ChatRecordSerializer.Vote(
                        data={
                            "vote_status": request.data.get("vote_status"),
                            "chat_id": chat_id,
                            "chat_record_id": chat_record_id,
                        }
                    ).vote()
        ...
        class ChatRecordImprove(APIView):
            ...
            def get(
                ...
                return result.success(
                    ChatRecordSerializer.ChatRecordImprove(
        ...
        class Improve(APIView):
            ...
            class Operate(APIView):
                ...
                def delete(
                    ...
                    return result.success(
                        ChatRecordSerializer.Improve.Operate(
                            data={
                                "chat_id": chat_id,
                                "chat_record_id": chat_record_id,
                                "dataset_id": dataset_id,
