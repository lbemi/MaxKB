...
def add_access_num(client_id=None, client_type=None):
    if client_type == AuthenticationType.APPLICATION_ACCESS_TOKEN.value:
        application_public_access_client = (
            QuerySet(ApplicationPublicAccessClient).filter(id=client_id).first()
        )
...
class BaseChatStep(IChatStep):
        ...
        **kwargs,
        ...
        chat_model = (
            get_model_instance_by_model_user_id(
                model_id, user_id, **model_params_setting
            )
        ...
        if chat_model is None:
            return (
                iter(
                    [
                        AIMessageChunk(
                            "抱歉，没有配置 AI 模型，无法优化引用分段，请先去应用中设置 AI 模型。"
                        )
                    ]
                ),
                False,
    ...
    def get_block_result(
        ...
        directly_return_chunk_list = [
            AIMessage(content=paragraph.content)
            for paragraph in paragraph_list
            if paragraph.hit_handling_method == "directly_return"
        ]
        if (
            directly_return_chunk_list is not None
            and len(directly_return_chunk_list) > 0
