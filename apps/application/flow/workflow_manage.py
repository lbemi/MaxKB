...
class Flow:
    ...
    def is_valid_node(self, node: Node):
        ...
        if node.type == "condition-node":
            ...
            if len(edge_list) == 0 and not end_nodes.__contains__(node.type):
                ...
                raise AppApiException(
                    500, f'{node.properties.get("stepName")} 节点不能连接俩个节点'
                )
    ...
    def get_next_nodes(self, node: Node):
        ...
        node_list = reduce(
            lambda x, y: [*x, *y],
            [
                [node for node in self.nodes if node.id == edge.targetNodeId]
                for edge in edge_list
            ],
            [],
        )
        if len(node_list) == 0 and not end_nodes.__contains__(node.type):
            raise AppApiException(500, f"不存在的下一个节点")
        return node_list
    ...
    def is_valid_model_params(self):
        node_list = [
            ...
            if (node.type == "ai-chat-node" or node.type == "question-node")
        ]
        for node in node_list:
            ...
            if model is None:
                raise ValidationError(
                    ErrorDetail(f'节点{node.properties.get("stepName")} 模型不存在')
                )
            ...
            if node.properties.get("status", 200) != 200:
                raise ValidationError(
                    ErrorDetail(f'节点{node.properties.get("stepName")} 不可用')
                )
        ...
        for node in node_list:
            ...
            if f_lib is None:
                raise ValidationError(
                    ErrorDetail(f'节点{node.properties.get("stepName")} 函数库不可用')
                )
...
class WorkflowManage:
        ...
        try:
            while self.has_next_node(self.current_result):
                ...
                if not self.has_next_node(self.current_result):
                    return tools.to_response_simple(
                        self.params["chat_id"],
                        self.params["chat_record_id"],
                        AIMessage(self.answer),
                        self,
                        self.work_flow_post_handler,
                    )
        except Exception as e:
            return tools.to_response(
                self.params["chat_id"],
                self.params["chat_record_id"],
                AIMessage(str(e)),
                self,
    ...
    def stream_event(self):
        try:
            ...
            self.work_flow_post_handler.handler(
                self.params["chat_id"], self.params["chat_record_id"], self.answer, self
            )
        except Exception as e:
            self.current_node.get_write_error_context(e)
            self.answer += str(e)
    ...
    def is_result(self):
        ...
        """
        return (
            self.current_node.node_params.get(
                "is_result", not self.has_next_node(self.current_result)
            )
            if self.current_node.node_params is not None
            else False
        )
        ...
        if self.current_node is None:
            ...
            if node_result is not None and node_result.is_assertion_result():
                for edge in self.flow.edges:
                    if (
                        edge.sourceNodeId == self.current_node.id
    ...
    def get_next_node(self):
        ...
        if (
            ...
            and self.current_result.is_assertion_result()
        ):
            for edge in self.flow.edges:
                if (
                    edge.sourceNodeId == self.current_node.id
                    and f"{edge.sourceNodeId}_{self.current_result.node_variable.get('branch_id')}_right"
                    == edge.sourceAnchorId
                ):
        ...
        for node in self.node_context:
            ...
            if node_config is not None:
                ...
                if fields is not None:
                    for field in fields:
                        globeLabel = (
                            f"{properties.get('stepName')}.{field.get('value')}"
                        )
