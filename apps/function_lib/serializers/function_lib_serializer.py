...
class FunctionLibInputField(serializers.Serializer):
    ...
    is_required = serializers.BooleanField(
        required=True, error_messages=ErrMessage.boolean("是否必填")
    )
    type = serializers.CharField(
        ...
        validators=[
            validators.RegexValidator(
                ...
                message="字段只支持string|int|dict|array|float",
                code=500,
            )
        ],
    )
    source = serializers.CharField(
        required=True,
        error_messages=ErrMessage.char("来源"),
        validators=[
            validators.RegexValidator(
                regex=re.compile("^custom|reference$"),
                message="字段只支持custom|reference",
                code=500,
            )
        ],
    )
...
class DebugInstance(serializers.Serializer):
    ...
    code = serializers.CharField(
        required=True, error_messages=ErrMessage.char("函数内容")
    )
...
class CreateFunctionLib(serializers.Serializer):
    name = serializers.CharField(
        required=True, error_messages=ErrMessage.char("函数名称")
    )
...
class FunctionLibSerializer(serializers.Serializer):
    class Query(serializers.Serializer):
        ...
        def list(self, with_valid=True):
            ...
            return [
                FunctionLibModelSerializer(item).data for item in self.get_query_set()
            ]
        ...
        def convert_value(name: str, value: str, _type: str, is_required: bool):
            ...
            try:
                ...
                raise AppApiException(
                    500, f"字段:{name}类型:{_type}值:{value}类型转换错误"
                )

    class Operate(serializers.Serializer):
        id = serializers.UUIDField(
            required=True, error_messages=ErrMessage.uuid("函数id")
        )
        user_id = serializers.UUIDField(
            required=True, error_messages=ErrMessage.uuid("用户id")
        )
