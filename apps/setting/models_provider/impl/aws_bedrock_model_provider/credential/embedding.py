    ...
    def _validate_model_type(cls, model_type: str, provider) -> bool:
        ...
        if not any(mt.get("value") == model_type for mt in model_type_list):
            raise AppApiException(
                ValidCode.valid_error.value, f"{model_type} 模型类型不支持"
            )
        return True

    @classmethod
    def _validate_credential(
        cls, model_credential: Dict[str, object]
    ) -> credential.Credential:
        for key in ["SecretId", "SecretKey"]:
            if key not in model_credential:
                raise AppApiException(
                    ValidCode.valid_error.value, f"{key} 字段为必填字段"
                )
        return credential.Credential(
            model_credential["SecretId"], model_credential["SecretKey"]
        )

    @classmethod
    def _test_credentials(cls, client, model_name: str):
        req = models.GetEmbeddingRequest()
        params = {"Model": model_name, "Input": "测试"}
        ...
        try:
            ...
            raise AppApiException(
                ValidCode.valid_error.value, f"校验失败,请检查参数是否正确: {str(e)}"
            )
