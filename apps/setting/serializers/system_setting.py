    ...
    class EmailSerializer(serializers.Serializer):
        ...
        class Create(serializers.Serializer):
            email_host = serializers.CharField(
                required=True, error_messages=ErrMessage.char("SMTP 主机")
            )
            email_port = serializers.IntegerField(
                required=True, error_messages=ErrMessage.char("SMTP 端口")
            )
            email_host_user = serializers.CharField(
                required=True, error_messages=ErrMessage.char("发件人邮箱")
            )
            ...
            email_use_tls = serializers.BooleanField(
                required=True, error_messages=ErrMessage.char("是否开启TLS")
            )
            email_use_ssl = serializers.BooleanField(
                required=True, error_messages=ErrMessage.char("是否开启SSL")
            )
            from_email = serializers.EmailField(
                required=True, error_messages=ErrMessage.char("发送人邮箱")
            )
