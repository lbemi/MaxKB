    ...
    return openapi.Schema(
        ...
        properties={
            ...
            "username": openapi.Schema(
                type=openapi.TYPE_STRING, title="用户名", description="用户名"
            ),
            "email": openapi.Schema(
                type=openapi.TYPE_STRING, title="邮箱", description="邮箱地址"
            ),
            "role": openapi.Schema(
                type=openapi.TYPE_STRING, title="角色", description="角色"
            ),
            "is_active": openapi.Schema(
                type=openapi.TYPE_STRING, title="是否可用", description="是否可用"
            ),
            "team_id": openapi.Schema(
                type=openapi.TYPE_STRING, title="团队id", description="团队id"
            ),
            "member_id": openapi.Schema(
                type=openapi.TYPE_STRING, title="成员id", description="成员id"
            ),
        },
    )
...
class TeamMemberPermissionOperate(ApiMixin, serializers.Serializer):
    USE = serializers.BooleanField(
        required=True, error_messages=ErrMessage.boolean("使用")
    )
    MANAGE = serializers.BooleanField(
        required=True, error_messages=ErrMessage.boolean("管理")
    )
...
class UpdateTeamMemberItemPermissionSerializer(ApiMixin, serializers.Serializer):
    target_id = serializers.CharField(
        required=True, error_messages=ErrMessage.char("目标id")
    )
    type = serializers.CharField(
        required=True, error_messages=ErrMessage.char("目标类型")
    )
...
class UpdateTeamMemberPermissionSerializer(ApiMixin, serializers.Serializer):
    team_member_permission_list = UpdateTeamMemberItemPermissionSerializer(
    ...
    def is_valid(self, *, user_id=None):
        ...
        if illegal_target_id_list is not None and len(illegal_target_id_list) > 0:
            raise AppApiException(
                500, "不存在的 应用|知识库id[" + str(illegal_target_id_list) + "]"
            )
    ...
    def update_or_save(self, member_id: str):
        ...
        for item in team_member_permission_list:
            exist_list = list(
                filter(
                    lambda use: str(use.target) == item.get("target_id"),
                    team_member_permission_exist_list,
                )
            )
            if len(exist_list) > 0:
                exist_list[0].operate = list(
                    filter(
                        lambda key: item.get("operate").get(key),
                ...
                save_list.append(
                    TeamMemberPermission(
                        ...
                        auth_target_type=item.get("type"),
                        operate=list(
                            filter(
                                lambda key: item.get("operate").get(key),
                                item.get("operate").keys(),
    ...
    def get_request_body_api(self):
        return openapi.Schema(
            ...
            required=["id"],
            properties={
                "team_member_permission_list": openapi.Schema(
...
class TeamMemberSerializer(ApiMixin, serializers.Serializer):
    team_id = serializers.UUIDField(
        required=True, error_messages=ErrMessage.uuid("团队id")
    )
    ...
    def batch_add_member(self, user_id_list: List[str], with_valid=True):
        ...
        use_user_id_list = [
            str(u.id) for u in QuerySet(User).filter(id__in=user_id_list)
        ]

        team_member_user_id_list = [
            str(team_member.user_id)
            for team_member in QuerySet(TeamMember).filter(
                team_id=self.data.get("team_id")
            )
        ]
        team_id = self.data.get("team_id")
        create_team_member_list = [
            self.to_member_model(
                add_user_id, team_member_user_id_list, use_user_id_list, team_id
            )
            for add_user_id in user_id_list
        ]
        (
            QuerySet(TeamMember).bulk_create(create_team_member_list)
            if len(create_team_member_list) > 0
            else None
        )
        return TeamMemberSerializer(
            data={"team_id": self.data.get("team_id")}
        ).list_member()

    def to_member_model(
        self, add_user_id, team_member_user_id_list, use_user_id_list, user_id
    ):
        if use_user_id_list.__contains__(add_user_id):
            if (
                team_member_user_id_list.__contains__(add_user_id)
                or user_id == add_user_id
            ):
    ...
    def add_member(self, username_or_email: str, with_valid=True):
        ...
        if user is None:
            raise AppApiException(500, "不存在的用户")
        if QuerySet(TeamMember).filter(
            Q(team_id=self.data.get("team_id")) & Q(user=user)
        ).exists() or self.data.get("team_id") == str(user.id):
            raise AppApiException(500, "团队中已存在当前成员,不要重复添加")
    ...
    def list_member(self, with_valid=True):
        ...
        return [
            {
                "id": "root",
                "email": manage_member.email,
                "username": manage_member.username,
                "team_id": self.data.get("team_id"),
                "user_id": manage_member.id,
                "type": "manage",
            },
            *member_list,
        ]
    ...
    class Operate(ApiMixin, serializers.Serializer):
        ...
        member_id = serializers.CharField(
            required=True, error_messages=ErrMessage.char("成员id")
        )
        # 团队id
        team_id = serializers.CharField(
            required=True, error_messages=ErrMessage.char("团队id")
        )

        def is_valid(self, *, raise_exception=True):
            super().is_valid(raise_exception=True)
            if (
                self.data.get("member_id") != "root"
                and not QuerySet(TeamMember)
                .filter(team_id=self.data.get("team_id"), id=self.data.get("member_id"))
                .exists()
            ):
        ...
        def list_member_permission(self, with_valid=True):
            ...
            member_permission_list = select_list(
                ...
                [
                    team_id,
                    team_id,
                    (member_id if member_id != "root" else uuid.uuid1()),
                ],
            )
                ...
                member_permission_list = list(
                    map(
                        lambda row: {
                            **row,
                            "operate": {
                                Operate.USE.value: True,
                                Operate.MANAGE.value: True,
                            },
                        },
                                    ...
                                    map(
                                        lambda key: (
                                            ...
                                            (
                                                ...
                                                and m.get("operate").__contains__(key)
                                                else False
                                            ),
                                        ),
