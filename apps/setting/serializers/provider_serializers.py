...
class ModelSerializer(serializers.Serializer):
    class Query(serializers.Serializer):
        ...
        name = serializers.CharField(
            required=False, max_length=64, error_messages=ErrMessage.char("模型名称")
        )

        model_type = serializers.CharField(
            required=False, error_messages=ErrMessage.char("模型类型")
        )

        model_name = serializers.CharField(
            required=False, error_messages=ErrMessage.char("基础模型")
        )

        provider = serializers.CharField(
            required=False, error_messages=ErrMessage.char("供应商")
        )
        ...
        def list(self, with_valid):
            ...
            model_query_set = QuerySet(Model).filter(
                (Q(user_id=user_id) | Q(permission_type="PUBLIC"))
            )
            query_params = {}
            if name is not None:
            ...
            return [
                ...
                for model in model_query_set.filter(**query_params).order_by(
                    "-create_time"
                )
            ]
    ...
    class Edit(serializers.Serializer):
        ...
        permission_type = serializers.CharField(
            required=False,
            error_messages=ErrMessage.char("权限"),
            validators=[
                validators.RegexValidator(
                    regex=re.compile("^PUBLIC|PRIVATE$"),
                    message="权限只支持PUBLIC|PRIVATE",
                    code=500,
                )
            ],
        )
        ...
        credential = serializers.DictField(
            required=False, error_messages=ErrMessage.dict("认证信息")
        )
            ...
            if "name" in self.data and self.data.get("name") is not None:
                ...
                if (
                    ...
                    raise AppApiException(
                        500, f'模型名称【{self.data.get("name")}】已存在'
                    )
    ...
    class Create(serializers.Serializer):
        ...
        name = serializers.CharField(
            required=True, max_length=64, error_messages=ErrMessage.char("模型名称")
        )

        provider = serializers.CharField(
            required=True, error_messages=ErrMessage.char("供应商")
        )

        model_type = serializers.CharField(
            required=True, error_messages=ErrMessage.char("模型类型")
        )
        ...
        model_name = serializers.CharField(
            required=True, error_messages=ErrMessage.char("基础模型")
        )

        credential = serializers.DictField(
            required=True, error_messages=ErrMessage.dict("认证信息")
        )
        ...
        def is_valid(self, *, raise_exception=False):
            ...
            if (
                QuerySet(Model)
                .filter(user_id=self.data.get("user_id"), name=self.data.get("name"))
                .exists()
            ):
        ...
        def insert(self, user_id, with_valid=False):
            ...
            if status == Status.DOWNLOAD:
                ...
                thread.start()
            return ModelSerializer.Operate(
                data={"id": model.id, "user_id": user_id}
            ).one(with_valid=True)
    ...
    class ModelParams(serializers.Serializer):
        id = serializers.UUIDField(
            required=True, error_messages=ErrMessage.uuid("模型id")
        )
        ...
        def is_valid(self, *, raise_exception=False):
            ...
            if model.permission_type == PermissionType.PRIVATE and self.data.get(
                "user_id"
            ) != str(model.user_id):
        ...
        def get_model_params(self, with_valid=True):
            ...
            return credential.get_model_params_setting_form(
                model.model_name
            ).to_form_list()
    ...
    class Operate(serializers.Serializer):
        ...
        def is_valid(self, *, raise_exception=False):
            ...
            model = (
                QuerySet(Model)
                .filter(id=self.data.get("id"), user_id=self.data.get("user_id"))
                .first()
            )
            if model is None:
                raise AppApiException(500, "模型不存在")
        ...
        def delete(self, with_valid=True):
            ...
            if model.model_type == "LLM":
                ...
                if application_count > 0:
                    raise AppApiException(
                        500, f"该模型关联了{application_count} 个应用，无法删除该模型。"
                    )
            elif model.model_type == "EMBEDDING":
                dataset_count = DataSet.objects.filter(
                    embedding_mode_id=model_id
                ).count()
                if dataset_count > 0:
                    raise AppApiException(
                        500, f"该模型关联了{dataset_count} 个知识库，无法删除该模型。"
                    )
            model.delete()
            return True
...
class ProviderSerializer(serializers.Serializer):
    ...
    method = serializers.CharField(
        required=True, error_messages=ErrMessage.char("执行函数名称")
    )
