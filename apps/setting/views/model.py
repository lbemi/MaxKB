...
from setting.swagger_api.provide_api import (
    ProvideApi,
    ModelCreateApi,
    ModelQueryApi,
    ModelEditApi,
...
class Model(APIView):
    ...
    def post(self, request: Request):
        return result.success(
            ModelSerializer.Create(
                data={**request.data, "user_id": str(request.user.id)}
            ).insert(request.user.id, with_valid=True)
        )

    @action(methods=["PUT"], detail=False)
    @swagger_auto_schema(
        operation_summary="下载模型,只试用与Ollama平台",
    ...
    class ModelMeta(APIView):
        ...
        @action(methods=["GET"], detail=False)
        @swagger_auto_schema(
            operation_summary="查询模型meta信息,该接口不携带认证信息",
        ...
        def get(self, request: Request, model_id: str):
            return result.success(
                ModelSerializer.Operate(
                    data={"id": model_id, "user_id": request.user.id}
    ...
    class PauseDownload(APIView):
        ...
        @action(methods=["PUT"], detail=False)
        @swagger_auto_schema(
            operation_summary="暂停模型下载", operation_id="暂停模型下载", tags=["模型"]
        )
        @has_permissions(PermissionConstants.MODEL_CREATE)
...
class Provide(APIView):
    ...
    class Exec(APIView):
        ...
        @action(methods=["POST"], detail=False)
        @swagger_auto_schema(
            operation_summary="调用供应商函数,获取表单数据",
        ...
        def post(self, request: Request, provider: str, method: str):
            return result.success(
                ProviderSerializer(data={"provider": provider, "method": method}).exec(
    ...
    class ModelTypeList(APIView):
        ...
        @swagger_auto_schema(
            ...
            responses=result.get_api_array_response(
                ProvideApi.ModelTypeList.get_response_body_api()
            ),
            tags=["模型"],
        )
        @has_permissions(PermissionConstants.MODEL_READ)
    ...
    class ModelList(APIView):
        ...
        @swagger_auto_schema(
            ...
            responses=result.get_api_array_response(
                ProvideApi.ModelList.get_response_body_api()
            ),
            tags=["模型"],
        )
        @has_permissions(PermissionConstants.MODEL_READ)
