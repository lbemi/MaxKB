...
from setting.serializers.team_serializers import (
...
class TeamMember(APIView):
    ...
    def get(self, request: Request):
        return result.success(
            TeamMemberSerializer(data={"team_id": str(request.user.id)}).list_member()
        )

    @action(methods=["POST"], detail=False)
    @swagger_auto_schema(
        operation_summary="添加成员",
        operation_id="添加成员",
    ...
    class Operate(APIView):
        ...
        def get(self, request: Request, member_id: str):
            return result.success(
                TeamMemberSerializer.Operate(
                    data={"member_id": member_id, "team_id": str(request.user.id)}
                ).list_member_permission()
            )

        @action(methods=["PUT"], detail=False)
        @swagger_auto_schema(
            operation_summary="修改团队成员权限",
